...
If your shared `mcp_servers` checkout is not in the default sibling location, set `MCP_SERVERS_ROOT` first.
...
- Runtime source: imports the shared launcher runtime from `mcp_servers`, but canonical wire ownership and bootstrap logic are here.
...
- Serial autonomous Fixer loop:
  ...
  - Headless/manual Codex launches now persist proxy-related env state in `.codex/runtime_proxy_env.json`. If a later daemon/GUI-launched worker lacks `ALL_PROXY` / `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY`, the wire reuses the last saved snapshot instead of depending on the parent shell's env inheritance.
