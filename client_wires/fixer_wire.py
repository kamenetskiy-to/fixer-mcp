...
def _build_backend_launch_env(
    ...
    *,
    ...
    if normalized_backend in {"droid", "claude"}:
        launch_env.clear_proxy_env(env)
    elif cwd is not None:
        launch_env.apply_proxy_env(env, launch_env.resolve_proxy_env(cwd, os.environ))
    return env
...
def _resolve_latest_fixer_resume_session_id(cwd: Path) -> str:
    summaries = _load_fixer_resume_summaries(cwd, limit=1)
    if not summaries:
        raise RuntimeError("No existing Fixer sessions were found for this project cwd.")
...
def _resolve_netrunner_launch_selection(
    ...
    *,
    ...
    return SessionLaunchSelection(
        backend=backend,
        model=_normalize_backend_model(descriptor, model),
        reasoning=_normalize_backend_reasoning(descriptor, reasoning),
    )
...
def _select_fresh_launch_selection(
    *,
    ...
    backend = preferred_backend if preset_backend else _select_backend_interactive(preferred_backend, Option, single_select_items)
    descriptor = _backend_descriptor(backend)
...
def _select_role_preset_server_names(
    ...
    *,
    ...
    if role == "overseer":
        selected_names = [name for name in selected_names if name != FIGMA_CONSOLE_MCP_NAME]
    ...
    if FORCED_MCP_SERVER in available_servers:
        selected_names.append(FORCED_MCP_SERVER)
    return _normalize_names(selected_names)
...
def _launch_fresh_role_session(
    ...
    *,
    ...
    cwd = Path.cwd()
    launch_selection = _select_fresh_launch_selection(
    ...
    selected_servers = {
        name: dict(available_servers[name]) for name in _normalize_names(selected_mcp_names) if name in available_servers
    }
    missing_names = [name for name in _normalize_names(selected_mcp_names) if name not in available_servers]
    if missing_names:
    ...
    if normalize_backend_name(getattr(adapter, "name", "")) == "codex":
        codex_args.extend(["--disable", "apps"])
    ...
    if prompt:
        command.extend(adapter.build_prompt_args(prompt))

    env = _build_backend_launch_env(
        adapter,
        llm_selection,
        cwd=cwd,
...
def _launch_fixer(
    ...
    *,
    ...
    if preset_resume_session_id:
        resume_codex_session_id = str(preset_resume_session_id).strip()
        if not resume_codex_session_id:
            raise RuntimeError("Explicit fixer session id must be non-empty.")
    elif preset_resume_latest:
        resume_codex_session_id = _resolve_latest_fixer_resume_session_id(cwd)
    else:
        ...
        if launch_mode == FIXER_LAUNCH_RESUME:
            ...
            available_servers, _config_env_vars, _adapter, _ensure_sqlite_scaffold = _load_available_servers(cwd)
            if FORCED_MCP_SERVER in available_servers:
            ...
            return _launch_fresh_role_session(
                "fixer",
                _build_fixer_prompt(),
                passthrough_args,
    ...
    if resume_codex_session_id:
        if normalize_backend_name(getattr(adapter, "name", "")) == "codex":
            codex_cmd = [adapter.command, "fork", *option_args, resume_codex_session_id]
        else:
...
def _launch_netrunner(
    ...
    *,
    ...
    selected_servers = _bind_fixer_db_path_to_server_env(selected_servers, db_path=db_path)
    selected_servers = _bind_netrunner_stateless_auth_to_server_env(selected_servers, project_cwd=cwd)
    selected_servers = _bind_launcher_telegram_env_to_server_env(selected_servers)
    ...
    if selected_session.status != "pending":
        descriptor = _backend_descriptor(launch_selection.backend)
...
def _launch_overseer(
    ...
    *,
    ...
    cwd = Path.cwd()
    available_servers, _config_env_vars, _adapter, _ensure_sqlite_scaffold = _load_available_servers(cwd)
    selected_mcp_names = _select_role_preset_server_names(available_servers, cwd=cwd, role="overseer")
    return _launch_fresh_role_session(
        "overseer",
        _build_overseer_prompt(),
        passthrough_args,
        selected_mcp_names=selected_mcp_names,
        dry_run=dry_run,
        preset_backend=None,
