...
func WakeFixerAutonomous(ctx context.Context, req *mcp.CallToolRequest, input WakeFixerAutonomousInput) (*mcp.CallToolResult, WakeFixerAutonomousOutput, error) {
	...
	launcherScript := filepath.Clean(filepath.Join("..", "client_wires", "fixer_autonomous.py"))
	if _, statErr := os.Stat(launcherScript); statErr != nil {
		return &mcp.CallToolResult{IsError: true}, WakeFixerAutonomousOutput{}, fmt.Errorf("autonomous launcher script unavailable: %v", statErr)
	}
