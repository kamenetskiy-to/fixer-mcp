...
- dedicated image-generation launch/wait/copy primitives for Codex sidecar jobs
...
Fixer can now delegate image creation or editing to a separate Codex subprocess without blocking the main Fixer thread context:

- `launch_image_generation_job` starts a durable job with `codex exec --enable image_generation` and can attach local input images for edit flows
- `wait_for_image_generation_job` resolves the generated file path from JSON events when possible, then falls back to the last message and `~/.codex/generated_images`
- `copy_image_generation_job_output` copies a completed image into the project workspace when the Fixer wants a repo-local asset
