...
void main() {
  testWidgets(
    'hands selector is constrained to the real selected thread provider+model',
    (tester) async {
      ...
      expect(selector.providers, hasLength(1));
      expect(selector.providers.single.id, 'codex');
      expect(selector.providers.single.models, ['deepseek-v4-flash']);
      expect(selector.selectedProvider, 'codex');
      expect(selector.selectedModel, 'deepseek-v4-flash');
      expect(selector.selectedReasoning, 'high');
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        selector.providers.map((provider) => provider.id),
...
ProjectWorkroomStore _selectorStore() {
  final fixture = workroomFixture();
  return ProjectWorkroomStore(
...
class _FakeSelectorRepository implements ProjectWorkroomRepository {
  ...
  Stream<ProjectUiFrame> watchProjectUi(
    ...
    yield const ProjectUiHeartbeatFrame(
